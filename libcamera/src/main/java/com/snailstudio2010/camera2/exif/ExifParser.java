...
package com.snailstudio2010.camera2.exif;
...
class ExifParser {
    ...
    protected int next() throws IOException, ExifInvalidFormatException {
        ...
        if (offset < endOfTags) {
            ...
            if (mIfdType == IfdId.TYPE_IFD_0) {
                ...
                if (offsetSize < 4) {
                    Logger.e(TAG, "Invalid size of link to next IFD: " + offsetSize);
                } else {
                    long ifdOffset = readUnsignedLong();
                    if (ifdOffset != 0) {
                        Logger.e(TAG, "Invalid link to next IFD: " + ifdOffset);
                    }
                }
            }
        }
        while (mCorrespondingEvent.size() != 0) {
            ...
            try {
                ...
                Logger.e(TAG, "error to skip to data at: " + entry.getKey() +
            ...
            if (event instanceof IfdEvent) {
                ...
                if (mNumOfTagInIfd * TAG_SIZE + mIfdStartOffset + OFFSET_SIZE > mApp1End) {
                    Logger.e(TAG, "Invalid size of IFD " + mIfdType);
                    return EVENT_END;
                }
    ...
    private ExifTag readTag() throws IOException, ExifInvalidFormatException {
        ...
        if (!ExifTag.isValidType(dataFormat)) {
            Logger.e(TAG, String.format("Tag %04x: Invalid data type %d", tagId, dataFormat));
            mTiffStream.skip(4);
            return null;
        }
            ...
            if (mCorrespondingEvent.size() > 0) {
                if (mCorrespondingEvent.firstEntry().getKey() < mTiffStream.getReadByteCount()
                    ...
                    if (event instanceof ImageEvent) {
                        ...
                        Logger.e(TAG, "Thumbnail overlaps value for tag: \n" + tag.toString());
                        Entry<Integer, Object> entry = mCorrespondingEvent.pollFirstEntry();
                        Logger.e(TAG, "Invalid thumbnail offset: " + entry.getKey());
                    } else {
                        // Tag value overlaps another tag, shorten count
                        if (event instanceof IfdEvent) {
                            Logger.e(TAG, "Ifd " + ((IfdEvent) event).ifd
                                    + " overlaps value for tag: \n" + tag.toString());
                        } else if (event instanceof ExifTagEvent) {
                            Logger.e(TAG, "Tag value for tag: \n"
                                    + ((ExifTagEvent) event).tag.toString()
                                    + " overlaps value for tag: \n" + tag.toString());
                        }
                        size = mCorrespondingEvent.firstEntry().getKey()
                                - mTiffStream.getReadByteCount();
                        Logger.e(TAG, "Invalid size of tag: \n" + tag.toString()
    ...
    private boolean seekTiffData(InputStream inputStream) throws IOException,
        ...
        while (marker != JpegHeader.EOI
            ...
            if (length < 2 || (length - 2) != dataStream.skip(length - 2)) {
                Logger.e(TAG, "Invalid JPEG format.");
                return false;
            }
