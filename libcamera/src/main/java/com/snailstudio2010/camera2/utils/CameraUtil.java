...
public class CameraUtil {
    ...
    public static void sortCamera2Size(Size[] sizes) {
    ...
    public static Size findSize(Size[] sizes, Comparator<Size> comparator) {
        Size res = sizes[0];
        for (int i = 1; i < sizes.length; i++) {
            if (comparator.compare(sizes[i], res) > 0) res = sizes[i];
        }
        return res;
    }
     ...
     *
     * @param supportSize specific stream configuration map used for get supported preview size
    ...
    public static int getJpgRotation(CameraCharacteristics c, int deviceRotation) {
        ...
        if (sensorRotation == null || lensFace == null) {
            Logger.e(TAG, "can not get sensor rotation or lens face");
    ...
    public static String hardwareLevel2Sting(@Nullable Integer level) {
        switch (level) {
